...
fun TopBar() {
    ...
    Scaffold(
        modifier = Modifier.nestedScroll(scrollBehavior.nestedScrollConnection).width(361.dp)
            .height(45.dp),
        topBar = {
            CenterAlignedTopAppBar(
                ...
                title = { Text("Echo", maxLines = 1, overflow = TextOverflow.Ellipsis) },
                navigationIcon = {
...
fun ScrollContent(innerPadding: PaddingValues) {
    ...
    LazyColumn(
        ...
        items(range.count()) { index -> Text(text = "- List item number ${index + 1}") }
    }
}
